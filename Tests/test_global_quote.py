...
class Test_GlobalQuote(Base.BaseClass):

    @pytest.mark.skipif(Base.is_testcase_runnable('test_global_quote'), reason="skipping because flag is set to false in excel")
    def test_global_quote(self):
        ...
        # symbol validation
        self.assertion.assert_equal(response_json['Global Quote']['01. symbol'] , "IBM",
                         "Expected symbol is IBM but found "+ response_json['Global Quote']['01. symbol'])
    ...
    @pytest.mark.skipif(Base.is_testcase_runnable('test_global_quote_invalid_symbol'), reason="skipping because flag is set to false in excel")
    def test_global_quote_invalid_symbol(self):
        ...
        self.logger.info("response : " + response.text)
        # response assertion
        self.assertion.assert_equal(response_json['Global Quote'],
                        {},
                         "response not meeted expected error" + json.dumps(response_json['Global Quote']))
        self.logger.info("test_time_series_daily_invalid_symbol ended")
        self.assert_all()

    @pytest.mark.skipif(Base.is_testcase_runnable('test_global_quote_invalid_apiToken_and_symbol'), reason="skipping because flag is set to false in excel")
    ...
    @pytest.mark.skipif(Base.is_testcase_runnable('test_global_quote_without_symbol_parameter'), reason="skipping because flag is set to false in excel")
    ...
    @pytest.mark.skipif(Base.is_testcase_runnable('test_global_quote_without_apikey_parameter'), reason="skipping because flag is set to false in excel")
